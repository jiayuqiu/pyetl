...
do
  ...
  /usr/bin/spark-submit \
     ...
     core/etl/sdec_spark_gc.py --env=prod --date=${current_date} --data_source=snat --hos_series=D --data_type=gc --platname=大通 \
     > $workpath/log/D_spark_sdec_${current_date}.log
  ...
  /usr/bin/spark-submit \
     ...
     --py-files hdfs:///user/qjy20472/cloud/pyetl/core.zip \
     --jars $(echo $workpath/core/etl/jars/*.jar | tr ' ' ',') \
     core/etl/sdec_spark_gc.py --env=prod --date=${current_date} --data_source=snat --hos_series=ET --data_type=gc --platname=大通 \
     > $workpath/log/ET_spark_sdec_${current_date}.log
